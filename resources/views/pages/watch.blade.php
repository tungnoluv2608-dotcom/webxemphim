...
<div class="row container" id="wrapper">
         ...
         <main id="main-contents" class="col-xs-12 col-sm-12 col-md-8">
            ...
            <div class="title-wrapper-xem full" style="width:100%">
               <h5 class="entry-title">Phim : {{$movie->title}} | Tập {{$tapphim}}</h5>
            </div>
                                          ...
                                          <ul class="halim-list-eps">
                                             @foreach($episode_list as $key => $epi)
                                                ...
                                                <a href="{{url('xem-phim/'.$movie->slug.'/tap-'.$epi->episode.'/server-'.$epi->server)}}">
                                                   <li class="halim-episode"><span class="halim-btn halim-btn-2 {{$tapphim==$epi->episode && $server_active=='server-'.$ser->id ? 'active' : ''}} halim-info-1-1 box-shadow" title="Xem phim {{$movie->title}} - Tập {{$epi->episode}} - {{$movie->name_eng}} - Vietsub + Thuyết Minh" data-h1="{{$movie->title}} - tập {{$epi->episode}}">{{$epi->episode}}</span></li>
                                                </a>
                                                @endif
                                             @endforeach
                                          </ul>
